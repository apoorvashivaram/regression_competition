# regression competition - model set 3 ----
...
# recipes -----
money_recipe <- recipe(money_made_inv ~ out_prncp_inv + loan_amnt + int_rate +
                         term + grade + bc_util,
                       data = money_train) %>% 
...
save(money_fold, money_recipe,
     file = "final_models/model_info/money_setup_v3.rda")
...
# final model script to be opened & run as a job
# random forest -- rf_tuning_3.R
...
# random forest
load(file = "final_models/model_info/rf_tune_v3.rda")
...
# write_csv(final_rf_results, "final_models/kaggle_submission/rf_output_model_3.csv")
